...
export default function FormBasics() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6 p-2 sm:p-4">
      ...
      <div className="flex flex-col sm:flex-row items-stretch gap-2 sm:gap-3 mt-2 w-full">
        <button type="submit" className="bg-black text-white px-6 py-2 rounded text-sm font-semibold disabled:opacity-60 w-full sm:w-auto" disabled={!isDirty && progress !== 100}>Guardar</button>
        {saved && <span className="text-xs text-green-700 sm:self-center">¡Guardado!</span>}
      </div>
    </form>
  );
}
